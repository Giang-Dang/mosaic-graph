...
/// because <c>AddMosaicSubgraph</c> also calls
/// <c>AddApplicationService&lt;ILoggerFactory&gt;()</c>. Without that line the
/// schema fails to build at startup, in every one of the six services.
/// </para>
/// <para>
/// The logger category names the service rather than being a constant, because
/// six subgraphs writing <c>Mosaic.RequestTimeline</c> into one aggregated log
/// would be six things nobody can tell apart.
/// </para>
/// </remarks>
public sealed class RequestTimelineListener : ExecutionDiagnosticEventListener
{
    private readonly ILogger _logger;

    public RequestTimelineListener(ILoggerFactory loggerFactory)
        => _logger = loggerFactory.CreateLogger("Mosaic.RequestTimeline");
