...
/// <summary>
/// Creates Inventory's database if it is not there, and fills it with the same
/// stock levels every chapter of this book has printed.
/// </summary>
/// <remarks>
/// <para>
...
public sealed class InventoryDatabaseSeeder(
    ...
    InventorySeedData seed,
    ILogger<InventoryDatabaseSeeder> logger)
    : IHostedService
{
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(
                "MOSAIC_RESET_DATABASE is set. Dropping the inventory database and everything in it.");
            await db.Database.EnsureDeletedAsync(cancellationToken);
        }

        var created = await db.Database.EnsureCreatedAsync(cancellationToken);

        if (await db.StockLevels.AnyAsync(cancellationToken))
        {
            logger.LogInformation(
                "Inventory is already seeded ({Count} stock levels).",
                await db.StockLevels.CountAsync(cancellationToken));
            return;
        }

        db.StockLevels.AddRange(seed.StockLevels);

        await db.SaveChangesAsync(cancellationToken);

        logger.LogInformation(
            "Seeded {Count} stock levels into a {State} database.",
            seed.StockLevels.Count,
