...
| `ch18` | 18. The Wire, Completely | No service changes. `scripts/wire-cases.mjs` starts isolated routers and records the client-facing facts that a schema cannot say: APQ is a cache round trip rather than a safelist, enabled `@defer` is multipart, and gzip changes the representation so the response varies on `Accept-Encoding`. Both verification scripts run the cases. |
