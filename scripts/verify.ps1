...
.DESCRIPTION
    ...
    scripts/satisfiability-cases.mjs composes the schemas four times over,
    varying which of catalog's root fields exist and which is declared first,
    and asserts which route the composer blames each time. scripts/planner-cases.mjs
    starts routers configured to log their own plan cache key, and asserts which
...
# Chapter 17, and the same split one more time. satisfiability-cases.mjs
# composes files and reads what the composer blamed; planner-cases.mjs starts
# routers and reads what each one keyed its plan cache on. Neither has a Postman
# collection, because what they measure is invisible to a request and a response:
# the first is a message about a graph nobody can run, and the second is a hash
...
try {
    ...
    if (-not (Test-Path -LiteralPath $FederationGraph)) {
        ...
        # These two cases are the control: vary which root fields exist and
        # which is declared first, and watch the message move. They also count
        # how often the composer repeats itself, because chapter 9 read a line
        # total as a fault total once already.
        #
        # No Docker and no running service: this composes files.
        if (-not (Test-Path -LiteralPath $SatisfiabilityCases)) {
            Write-Skipped 'satisfiability cases' 'scripts/satisfiability-cases.mjs does not exist yet'
        } elseif (-not (Get-Command node -ErrorAction SilentlyContinue)) {
            Stop-Verify 'satisfiability cases' 'node is not on PATH; it is needed to run scripts/satisfiability-cases.mjs.'
        } else {
            & node $SatisfiabilityCases
            if ($LASTEXITCODE -ne 0) {
                Stop-Verify 'satisfiability cases' (Join-Lines @(
                    "scripts/satisfiability-cases.mjs exited with $LASTEXITCODE."
                    'Either catalog''s root fields moved, in which case the case is right and'
                    'chapter 17 needs the new order, or wgc changed which routes it reports and'
                    'how often. Both are findings; neither is a reason to loosen the assertion.'))
            }
            Write-Ok 'the composer blames the first route declared, and repeats itself as chapter 17 says'
        }
        ...
        if ($SkipRouter) {
            ...
            # Four chapters asked a form of this and none could answer it,
            # because a stock router publishes no cache metrics at all. These
            # cases take the sharper route instead: the router is started with
            # an access-log field carrying request.operation.queryPlanHash, so
            # it reports its own cache key for every request.
            #
            # Every assertion is an equivalence class or a count. The one
            # duration in the output is printed and never asserted, which is
            # decision 62's line.
            if (-not (Test-Path -LiteralPath $PlannerCases)) {
                Write-Skipped 'planner cases' 'scripts/planner-cases.mjs does not exist yet'
            } elseif (-not (Get-Command node -ErrorAction SilentlyContinue)) {
                Stop-Verify 'planner cases' 'node is not on PATH; it is needed to run scripts/planner-cases.mjs.'
            } else {
                & node $PlannerCases
                if ($LASTEXITCODE -ne 0) {
                    Stop-Verify 'planner cases' (Join-Lines @(
                        "scripts/planner-cases.mjs exited with $LASTEXITCODE."
                        'Something about normalization, the plan cache or entity batching has'
                        'changed. The output above says which case. A router upgrade that makes'
                        'field order stop splitting the cache key, or that stops deduplicating'
                        'representations, is a better router and a wrong chapter 17: rewrite the'
